...
AND NOT EXISTS ( -- Comprobamos que no hayan vuelto a ascender después del descenso
  SELECT 1
...
SELECT P.TEMPORADA, P.JORNADA, SUM(P.GOLES_LOCAL + P.GOLES_VISITANTE) AS GOLES
FROM (
  SELECT TEMPORADA
  FROM  TEMPORADA_DIVISION
  ORDER BY TEMPORADA DESC
  LIMIT 5 -- Las últimas cinco temporadas
)
JOIN  PARTIDO AS P ON T.TEMPORADA = P.TEMPORADA
